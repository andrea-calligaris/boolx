...
// Copyright (c) 2008-2021 Andrea Calligaris
...
const bool PRINT_NEW_LINE_AFTER_TERMINATION = true;
const bool DBG_SKIP_COMMENTS = true;
const bool DBG_SKIP_NON_EXECUTED_INSTRUCTIONS = true;
const bool DBG_SKIP_EMPTY_CHARACTERS = true;
const bool DBG_ONLY_PRINT_BITS_IN_READABLE_ORDER = true;
const bool DBG_SHOW_CURRENT_BITS = true;

enum CONDITION_TYPES { c_if, c_else };
enum JUMP_TYPES { call, jump };
enum ERRORS {
  OK = 0,
  ERR_STRING_TOO_LONG,
  ERR_MISPLACED_ELSE,
...
bool my_strcpy(char *destination, char *source, size_t max_length);
bool my_strcat(char *destination, char *source, size_t max_length);

int main(int argc, char *argv[]);
int process_arguments(int argc, char *argv[]);

void register_all_labels(FILE *source_program);
void execute_source_program_function(FILE *source_program,
                                     long long int from_pos);
void process_current_instruction(FILE *source_program);

void instruction_if_condition_common();
void instruction_if_condition_equal_to_1();
void instruction_if_condition_equal_to_null();
void instruction_else_condition();
void instruction_end_of_if_else_statement();
void instruction_go_to_next_cell();
void instruction_go_to_previous_cell();
void instruction_go_to_next_bit();
void instruction_go_to_previous_bit();
void instruction_go_to_first_cell();
void instruction_go_to_first_bit();
void instruction_set_bit_to_zero();
void instruction_set_bit_to_one();
void instruction_set_bit_to_null();
void instruction_set_all_bits_to_null_and_go_to_first_bit();
void instruction_print_cell_value_as_ASCII_character();
void instruction_get_ASCII_input_and_save_as_cell_value();
void instruction_global_queue_enqueue();
void instruction_global_queue_dequeue();
void instruction_select_next_label();
void instruction_select_previous_label();
void instruction_select_first_label();
void instruction_jump_to_label(FILE *source_program);
void instruction_call_function();
void instruction_return();

void dbg_print_labels();
void dbg_print_instruction();
void dbg_print_stack_info();
void dbg_print_n_cells(int n);
void dbg_print_n_global_cells(int n);
void dbg_print_cell_value_common(struct TBit *ignored_starting_bit,
                                 struct TBit *selected_bit);
void dbg_print_cell_value(struct TCell *cell);
void dbg_print_global_cell_value(struct TGlobalCell *gl_cell);

void clear_if_else_statements();
void free_local_function_memory();
void free_global_variables();
void free_cell_content(struct TCell *cell);
void free_global_cell_content(struct TGlobalCell *g_cell);

void output(struct TCell *cell);
void input(struct TCell *cell);

void process_errors();

struct TCell *first_memory_cell;
struct TGlobalCell *front_global_cell;
struct TLabel *first_label;
struct TCell *selected_cell;
struct TCell *prevCell;
struct TGlobalCell *back_global_cell;
struct TLabel *curr_label;
struct TIfElseStatement *current_if_else_statement;
struct TDebugState debug_state;
char *source_program_path;
char current_instruction;
long long int program_file_cursor_position = 0;
bool skip_instruction_because_of_if_else_statement;
bool last_instruction_was_a_function_call = false;
bool last_instruction_was_a_return = false;
long long int n_nested_comments;
short int error = OK;
bool debug = false;
...
struct TBit {
  bool1_t value;
  struct TBit *next;  // "null" bit value is determined by this parameter
  struct TBit *prev;
};

struct TCell {  // memory cell
  struct TBit ignored_starting_bit;
  // The first bit (front, LSB to MSB) always exists and is ignored; the actual
  // selected bit is therefore considered "selected_bit->next"; this is
  // confusing but it allows to have just a boolean for the bit value,
  // decreasing memory usage in case you use a theoretical processor or a
  // virtual machine where "bool" is actually treated as one single bit; the
  // alternative would be an integer to account for the "null" value. Using a
  // "bit field" structure is also an option but it's slower in computation and
  // you can't fit the null value anyway.
  struct TBit *selected_bit;
  struct TCell *next;
  struct TCell *prev;
};

struct TGlobalCell {                 // memory cell of the global queue
  struct TBit ignored_starting_bit;  // the first bit always exists and is
                                     // ignored; see TCell
...
void register_all_labels(FILE *source_program) {
  while (fscanf(source_program, "%1c", &current_instruction) == 1) {
    if (current_instruction == '{') {
      n_nested_comments++;
    } else if (current_instruction == '}' && n_nested_comments > 0) {
      n_nested_comments--;
    }
    ...
    if (n_nested_comments == 0) {
      if (current_instruction == ':') {
        ...
        struct TLabel *new_label = malloc(sizeof(struct TLabel));
        new_label->next = NULL;
        new_label->file_pos = program_file_cursor_position;
        // update pointers:
        if (first_label == NULL) {
...
void execute_source_program_function(FILE *source_program,
  ...
  while (fscanf(source_program, "%1c", &current_instruction) == 1) {
    ...
    if (error != OK) {
      ...
      backup_pointer_of_current_if_else_statement = current_if_else_statement;
      // call another function:
      execute_source_program_function(source_program, curr_label->file_pos);
      // restore:
      fseek(source_program, backup_source_program_pos, SEEK_SET);
      first_memory_cell = &cells;
      selected_cell = backup_pointer_of_the_selected_cell;
      current_if_else_statement = backup_pointer_of_current_if_else_statement;
...
void process_current_instruction(FILE *source_program) {
  ...
  if (current_if_else_statement == NULL) {
    skip_instruction_because_of_if_else_statement = false;
  } else if (current_if_else_statement->skip_this_block == false &&
...
void instruction_if_condition_common() {
  if (current_if_else_statement == NULL) {
    ...
    current_if_else_statement = malloc(sizeof(struct TIfElseStatement));
    ...
    // this new if-else statement can only be executed if the parent is being
    // executed:
  ...
  current_if_else_statement->type = c_if;
}
...
void instruction_else_condition() {
  if (current_if_else_statement == NULL) {
    ...
    if (current_if_else_statement->type != c_if) {
      error = ERR_MISPLACED_ELSE;
    } else {
      current_if_else_statement->type = c_else;
...
void instruction_end_of_if_else_statement() {
  if (current_if_else_statement == NULL) {
    ...
    if (current_if_else_statement->prev_nested == NULL) {
      ...
      struct TIfElseStatement *statement_to_delete = current_if_else_statement;
      current_if_else_statement = current_if_else_statement->prev_nested;
      current_if_else_statement->next_nested = NULL;
...
void instruction_go_to_first_cell() { selected_cell = first_memory_cell; }
...
void instruction_set_bit_to_one() {
  if (selected_cell->selected_bit->next == NULL) {
    ...
    selected_cell->selected_bit->next->prev = selected_cell->selected_bit;
  } else {
    selected_cell->selected_bit->next->value = true;
  }
}

void instruction_set_bit_to_null() { selected_cell->selected_bit->next = NULL; }
...
void instruction_global_queue_enqueue() {
  if (front_global_cell == NULL) {
    ...
    struct TGlobalCell *new_gl_cell = malloc(sizeof(struct TGlobalCell));
  ...
  struct TBit *curr_cell_curr_bit_tmp = &selected_cell->ignored_starting_bit;
...
void instruction_global_queue_dequeue() {
  ...
  if (front_global_cell->ignored_starting_bit.next == NULL) {
    ...
    curr_cell_starting_bit->next = front_global_cell->ignored_starting_bit.next;
    curr_cell_starting_bit->next->prev =
        curr_cell_starting_bit;  // (give it the new parent)
  }
  // and delete the global cell:
...
void instruction_return() { last_instruction_was_a_return = true; }
  ...
  if (debug) {
    ...
    if (first_label == NULL) {
      ...
      while (curr_label_test != NULL) {
        printf("\tLabel #%d: position: %llu\n", i, curr_label_test->file_pos);
...
void dbg_print_instruction() {
  ...
  if (n_nested_comments > 0 || current_instruction == '{') {
    my_strcpy(additional_info, "   (comment)", sizeof additional_info);
    is_comment = true;
    debug_state.instr_has_immediate_effect_in_memory = false;
  } else if (skip_instruction_because_of_if_else_statement) {
    my_strcpy(additional_info, "   (skipping execution)",
              sizeof additional_info);
    debug_state.instr_has_immediate_effect_in_memory = false;
  } else if (current_instruction == '\r') {
    my_strcpy(additional_info, "(carriage return)", sizeof additional_info);
    is_empty_character = true;
    debug_state.instr_has_immediate_effect_in_memory = false;
    show_symbol = false;
  } else if (current_instruction == '\n') {
    my_strcpy(additional_info, "(new line)", sizeof additional_info);
    is_empty_character = true;
    debug_state.instr_has_immediate_effect_in_memory = false;
    show_symbol = false;
  } else if (current_instruction == '\t') {
    ...
    debug_state.instr_has_immediate_effect_in_memory = false;
    show_symbol = false;
  } else if (current_instruction == '/' || current_instruction == '\\' ||
             current_instruction == '$' || current_instruction == ']' ||
             current_instruction == '?' || current_instruction == '"' ||
             current_instruction == '!' || current_instruction == ';' ||
             current_instruction == '\'')
  // leaving out @ on purpose: already skipped by how jumping works;
  // for the same reason, leaving out : on purpose: it will show the new memory
  {
    debug_state.instr_has_immediate_effect_in_memory = false;
  }
  ...
  if (DBG_SKIP_COMMENTS && is_comment) {
    return;
  } else if (DBG_SKIP_NON_EXECUTED_INSTRUCTIONS &&
  ...
  if (show_symbol) {
    printf("Next instruction: %c%s", current_instruction, additional_info);
  } else {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (i > 0) {
      ...
      if (debug_state.dbg_current_cell->next != NULL) {
        debug_state.dbg_current_cell = debug_state.dbg_current_cell->next;
        go_ahead = true;
      }
    }
    if (go_ahead) {
...
void dbg_print_n_global_cells(int n) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (go_ahead) {
      if (i == 0 && debug_state.dbg_curr_global_cell->next != NULL) {
        printf("- Global #%d (front): ", i);
      } else if (i > 0 && debug_state.dbg_curr_global_cell->next == NULL) {
        printf("- Global #%d (back):  ", i);
      } else {
        printf("- Global #%d:         ", i);
      }
      dbg_print_global_cell_value(debug_state.dbg_curr_global_cell);
      printf("\n");
    }
  }
}
...
void dbg_print_cell_value(struct TCell *cell) {
  dbg_print_cell_value_common(&cell->ignored_starting_bit, cell->selected_bit);
}
...
void clear_if_else_statements() {
  ...
  while (current_if_else_statement != NULL) {
    ...
    current_if_else_statement = current_if_else_statement->next_nested;
    free(statement_to_delete);
  }
}
...
void input(struct TCell *cell) {
  ...
  if (debug) {
    getchar();  // so it doesn't immediately trigger the next printing
    printf("\n");
  }
  ...
  if (error) {
    ...
    switch (error) {
      ...
      case ERR_MISPLACED_ELSE:
        ...
        break;
      case ERR_END_IF:
        fprintf(stderr, "unexpected end of IF condition or else statement");
        break;
      case ERR_LABEL_CURSOR_OUTSIDE_OF_BOUNDS:
        fprintf(stderr, "label pointer moved outside of bounds");
        break;
      case ERR_JUMP_BUT_NO_LABEL:
        fprintf(stderr,
                "call or jump to a label, but there is no label at all");
        break;
      case ERR_SEEK_PROGRAM_POSITION:
        fprintf(stderr,
                "can't read from the requested position after a jump or "
                "function call");
        break;
      case ERR_EMPTY_GLOBAL_STACK:
        fprintf(stderr, "tried to pop from the global stack but it's empty");
        break;
      case ERR_USER_INPUT:
    ...
    printf(".\n");
    error = OK;  // to prevent multiple prints
  }
}
